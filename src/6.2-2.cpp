...
void breadth_first_search(vector<vector<int>>& links, int start, int goal)
{
    ...
    from[start] = start;
    queue_vertex.push(start);
    while (!queue_vertex.empty()) {
        int vertex = queue_vertex.front();
        queue_vertex.pop();
        if (vertex == goal) {
            reachable = true;
            break;
...
int main(int argc, char const* argv[])
{
    ...
    for (int start = 0; start < vertex_count; start++) {
        for (int goal = 0; goal < vertex_count; goal++) {
            breadth_first_search(links, start, goal);
        }
    }
